...
void LoadFavorites(void)
{
    ...
    FILE *fp = fopen("/data/a7800ds.fav", "rb");
    if (fp)
    {
...
void SaveFavorites(void)
{
    ...
    FILE *fp = fopen("/data/a7800ds.fav", "wb");
    if (fp)
    {
...
void dsDisplayFiles(unsigned int NoDebGame,u32 ucSel)
{
  ...
  sprintf(szName,"%s","A=CHOOSE Y=HALT START=FAV B=BACK");
  dsPrintValue(16-strlen(szName)/2,23,0,szName);
  for (u8 ucBcl=0;ucBcl<17; ucBcl++)
  {
    ...
    if (ucGame < countpro) {
      ...
      else 
      {
        ...
        if (IsFavorite(proromlist[ucGame].filename))
        {
            dsPrintValue(0,5+ucBcl,(IsFavorite(proromlist[ucGame].filename) == 1) ? 1:0,(char*)"@");
        }
        else
        {
...
unsigned int dsWaitForRom(void)
{
  ...
  while (!bDone) {
    ...
    if (keysCurrent() & KEY_SELECT)
    {
        ...
        {
            ToggleFavorite(proromlist[ucFicAct].filename);
            dsDisplayFiles(firstRomDisplay,romSelected);
            SaveFavorites();
      ...
      case A7800_PLAYGAME:
        ...
        if (dampen == 0)
        {
          ...
          if (keys_pressed != last_keys_pressed)
          {
              ...
              {
                  if ( (keys_pressed & (KEY_R | KEY_X)) )  { if (myCartInfo.xOffset < 50)  myCartInfo.xOffset +=28; bRefreshXY = true; }
                  if ( (keys_pressed & (KEY_L | KEY_Y)) )  { if (myCartInfo.xOffset > 28)  myCartInfo.xOffset -=28; bRefreshXY = true; }
              }
